...
```
docker-compose build --build-arg MAKE_THREADS=4 && docker-compose up -d
```
...
### Default: use only single core to make (compile) process:
```
docker-compose -f docker-compose.yaml -f docker-compose.override.profiling.yaml up -d --build
```
### Configure number of threads for make (compile) process. Use 4 CPU cores example:
```
